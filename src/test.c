...
void test_all() {
    ...
    //test_warning(",,^n:!k+1n^:+k1..1", atom_integer_new(3));
    //test_warning("!k4+,,^n:!k+1n^:+k1..1k", atom_integer_new(7));
    ...
    test("?19?0?0?0000?123", atom_integer_new(9));
    //test_failure("!f^n:fn. ?1f01", atom_integer_new(0));
    ...
    test("\\L [sum][map]^n:**nnn.[range]09", atom_integer_new(1296));
}
